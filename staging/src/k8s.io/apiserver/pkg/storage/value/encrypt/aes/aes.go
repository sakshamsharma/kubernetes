...
func (t *cbc) TransformFromStorage(data []byte, context value.Context) ([]byte, bool, error) {
	...
	padding := int(c)
	size := len(result) - padding
	if padding == 0 || padding > len(result) {
...
func (t *cbc) TransformToStorage(data []byte, context value.Context) ([]byte, error) {
	...
	result := make([]byte, blockSize+len(data)+padding)
	iv := result[:blockSize]
	...
	copy(result[blockSize+len(data):], bytes.Repeat([]byte{byte(padding)}, padding))
